...
@app.post("/run_c_code")
async def run_c_code(code: str = Form(...), input_data: str = Form(""), filename: str = Form("userprog")):
    ...
    try:
        run_proc = subprocess.run(run_cmd, capture_output=True, text=True, timeout=10, input=input_data)
